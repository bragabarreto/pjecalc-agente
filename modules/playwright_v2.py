...
class PlaywrightAutomatorV2:
    ...
    def fase_liquidar_e_exportar(self) -> str | None:
        ...
        # Pós-processamento: corrigir parâmetros que a UI JSF rejeitou
        # (ex: HORAS EXTRAS 50% com tipoDaQuantidade=IMPORTADA_DO_CARTAO).
        # Quando linkParametrizar falha por view-state stale, o XML do PJC
        # sai com quantidade=INFORMADA/0. Aqui editamos o XML antes de
        # disponibilizar ao usuário.
        try:
            corrigido = self._pos_processar_pjc(dest)
            if corrigido:
                self.log(f"  ✓ PJC pós-processado: {corrigido} correções aplicadas")
        except Exception as e:
            self.log(f"  ⚠ Pós-processamento PJC falhou (não fatal): {e}")
        return self._pjc_path

    def _pos_processar_pjc(self, pjc_path) -> int:
        """Edita o XML dentro do .PJC para corrigir parâmetros que a UI
        rejeitou (ex: HE 50% com tipoDaQuantidade=IMPORTADA_DO_CARTAO).

        Retorna número de correções aplicadas.
        """
        import zipfile, io, re
        from pathlib import Path
        # Mapping: nome verba (uppercase) → enum TipoDeQuantidadeImportadaDoCartaoDePontoEnum
        # Conferido em pjecalc-negocio-2.14.0.jar (TipoDeQuantidadeImportadaDoCartaoDePontoEnum.class)
        MAP_CARTAO = {
            "HORAS EXTRAS 50%": "HORAS_EXTRAS_DIURNAS_E_NOTURNAS",
            "HORAS EXTRAS 100%": "HORAS_EXTRAS_DIURNAS_E_NOTURNAS",
            "ADICIONAL NOTURNO": "HORAS_NOTURNAS",
            "INTERVALO INTRAJORNADA": "INTRAJORNADA",
            "INTERVALO INTERJORNADA": "INTERJORNADA",
        }
        # Coletar verbas do JSON que devem ter IMPORTADA_DO_CARTAO
        verbas_alvo = {}  # {nome_uppercase: enum_value}
        for v in self.previa.verbas_principais:
            nome = (v.nome_pjecalc or getattr(v, "expresso_alvo", "") or "").upper()
            if not nome:
                continue
            try:
                f = v.parametros.formula_calculado
                if f and f.quantidade and f.quantidade.tipo.value == "IMPORTADA_DO_CARTAO":
                    # Determinar enum baseado no nome (se JSON tem tipo_cartao_ponto, usar; senão inferir)
                    tcp = getattr(f.quantidade, "tipo_cartao_ponto", None)
                    if tcp:
                        verbas_alvo[nome] = tcp
                    else:
                        # Inferir por nome (match parcial)
                        for k, v_enum in MAP_CARTAO.items():
                            if k in nome:
                                verbas_alvo[nome] = v_enum
                                break
            except Exception:
                continue
        if not verbas_alvo:
            return 0
        self.log(f"  ℹ Verbas alvo p/ pós-processamento: {list(verbas_alvo.keys())}")
        # Ler ZIP
        with zipfile.ZipFile(pjc_path, "r") as zf:
            names = zf.namelist()
            xml_name = names[0]  # PJC tem 1 arquivo XML
            xml_bytes = zf.read(xml_name)
        # Tentar decodificar como ISO-8859-1 (formato do PJC)
        xml_text = xml_bytes.decode("iso-8859-1")
        correcoes = 0
        for nome, enum_val in verbas_alvo.items():
            # Encontrar o bloco da verba e dentro dele o tag Quantidade
            # Padrão: <nome>NOME</nome>...<quantidade><Quantidade>...<tipo>X</tipo>...<tipoImportadadoDoCartaoDePonto>null</...>
            tag_nome = f"<nome>{nome.replace('%', '%')}</nome>"
            # XML usa entidades HTML para alguns caracteres (ex: &#186;)
            # Vamos procurar de forma tolerante
            idx = xml_text.upper().find(f"<NOME>{nome}</NOME>")
            if idx < 0:
                self.log(f"    ⚠ verba '{nome}' não encontrada no XML")
                continue
            # Limitar busca aos próximos 5000 chars (escopo da verba)
            bloco_fim = idx + 5000
            bloco = xml_text[idx:bloco_fim]
            # Substituir <tipo>INFORMADA</tipo> por <tipo>IMPORTADA_DO_CARTAO_DE_PONTO</tipo>
            # dentro do primeiro <Quantidade>...</Quantidade> encontrado APÓS o nome
            q_start = bloco.find("<Quantidade>")
            q_end = bloco.find("</Quantidade>", q_start) if q_start > 0 else -1
            if q_start < 0 or q_end < 0:
                self.log(f"    ⚠ bloco Quantidade não encontrado p/ '{nome}'")
                continue
            quantidade_xml = bloco[q_start:q_end + len("</Quantidade>")]
            novo_q = quantidade_xml
            # 1. tipo INFORMADA → IMPORTADA_DO_CARTAO_DE_PONTO
            novo_q = re.sub(r"<tipo>INFORMADA</tipo>",
                            "<tipo>IMPORTADA_DO_CARTAO_DE_PONTO</tipo>", novo_q)
            # 2. tipoImportadadoDoCartaoDePonto null → enum
            novo_q = re.sub(r"<tipoImportadadoDoCartaoDePonto>null</tipoImportadadoDoCartaoDePonto>",
                            f"<tipoImportadadoDoCartaoDePonto>{enum_val}</tipoImportadadoDoCartaoDePonto>", novo_q)
            if novo_q != quantidade_xml:
                xml_text = xml_text.replace(quantidade_xml, novo_q, 1)
                correcoes += 1
                self.log(f"    ✓ '{nome}': tipo→IMPORTADA + enum→{enum_val}")
        if correcoes == 0:
            return 0
        # Re-escrever ZIP
        xml_bytes_novo = xml_text.encode("iso-8859-1")
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
            zf.writestr(xml_name, xml_bytes_novo)
        Path(pjc_path).write_bytes(buf.getvalue())
        return correcoes
